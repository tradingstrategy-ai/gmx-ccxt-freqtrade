...
def main() -> None:
    ...
    execution_buffer: float = float(ccxt_cfg.get("executionBuffer", 2.2))
    ...
    logger.info("Using execution_buffer=%.1fx for cancel transaction", execution_buffer)

    if len(order_keys) == 1:
        result = canceller.cancel_order(order_keys[0], execution_buffer=execution_buffer)
    else:
